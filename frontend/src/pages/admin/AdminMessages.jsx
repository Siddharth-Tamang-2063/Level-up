  ...
  return (
    <AdminLayout>
      <div className="max-w-7xl">
        <div className="mb-8">
          <h1 className="font-sans text-2xl font-semibold text-white">Messages</h1>
                ...
                <button key={msg.id} onClick={() => selectMsg(msg)}
                  ...
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <div className="flex items-center gap-2">
                      {!msg.read ? <Mail size={12} className="text-blue-500 flex-shrink-0" /> : <MailOpen size={12} className="text-white/30 flex-shrink-0" />}
                      <p className={`text-sm font-sans ${!msg.read ? 'font-medium text-white' : 'text-white/70'}`}>{msg.name}</p>
                    </div>
                    <p className="text-[10px] text-white/30 font-sans whitespace-nowrap flex-shrink-0">{msg.date}</p>
                  </div>
                  <p className="text-xs font-sans font-medium text-white/70 mb-1 pl-5">{msg.subject}</p>
                  <p className="text-xs font-sans text-white/40 pl-5 line-clamp-2">{msg.message}</p>
                </button>
              ))}

              {messages.length === 0 && (
                <div className="p-8 text-center">
                  <p className="text-sm text-white/40 font-sans">No messages</p>
                </div>
              )}
            </div>
          </div>
            ...
            {selected ? (
              <div className="h-full flex flex-col">
                ...
                <div className="flex items-start justify-between mb-6 pb-6 border-b border-gray-100">
                  <div>
                    <h2 className="font-sans font-semibold text-white text-lg mb-1">{selected.subject || 'No Subject'}</h2>
                    <div className="flex items-center gap-3">
                      <p className="text-sm font-sans text-white/60">{selected.name}</p>
                      <span className="text-white/20">·</span>
                      <p className="text-sm text-sand font-sans">{selected.email}</p>
                    </div>
                    <div className="flex items-center gap-1 mt-1 text-xs text-white/30 font-sans">
                      <Clock size={10} />
                      {selected.date}
                    </div>
                  </div>
                  <button onClick={() => deleteMsg(selected.id)}
                    className="w-8 h-8 flex items-center justify-center text-white/30 hover:text-red-500 hover:bg-red-50 rounded transition-all">
                    <Trash2 size={14} />
                  </button>
                </div>
                ...
                <div className="flex-1 mb-6">
                  <div className="bg-gray-50 rounded p-5">
                    <p className="text-sm font-sans text-white/80 leading-relaxed">{selected.message}</p>
                  </div>
                </div>
                ...
                {replySent[selected.id] ? (
                  ...
                  <div>
                    <p className="text-xs tracking-widest uppercase font-sans font-medium text-white mb-3">Reply</p>
                    <textarea
                      value={replyText}
                      onChange={e => setReplyText(e.target.value)}
                      rows={4}
                      placeholder={`Reply to ${selected.name}...`}
                      className="w-full border border-gray-200 rounded px-4 py-3 text-sm font-sans text-white placeholder:text-white/30 focus:outline-none focus:border-sand transition-colors resize-none mb-3"
                    />
              ...
              <div className="h-full flex items-center justify-center text-center">
                <div>
                  <MailOpen size={40} className="text-white/15 mx-auto mb-4" />
                  <p className="font-sans text-white/40 text-sm">Select a message to read it</p>
                </div>
              </div>
            )}
          </div>
