...
export default function Cart() {
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-4 text-center">
        <ShoppingBag size={56} className="text-white/20 mb-6" strokeWidth={1} />
        <h2 className="font-display text-3xl font-light text-white mb-3">Your bag is empty</h2>
        <p className="text-white/50 font-sans mb-8 max-w-sm">
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 md:py-16">
      ...
      <div className="flex items-baseline justify-between mb-10">
        ...
        <span className="text-sm text-white/50 font-sans">{itemCount} {itemCount === 1 ? 'item' : 'items'}</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10 lg:gap-16">
        ...
        <div className="divide-y divide-charcoal/10">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <Link to={`/product/${item.slug}`}>
                        <h3 className="font-display text-lg md:text-xl font-light text-white leading-tight hover:text-sand transition-colors">
                          {item.name}
                        </h3>
                      </Link>
                      <p className="text-sm text-white/50 font-sans mt-1">Size: {item.size}</p>
                    </div>
                    <button
                  ...
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <div className="flex items-center border border-charcoal/20">
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-white/60 hover:text-white hover:bg-cream-200 transition-colors"
                      >
                        <Minus size={12} />
                      </button>
                      <span className="w-8 h-8 flex items-center justify-center text-sm font-sans text-white border-x border-charcoal/20">
                        {item.qty}
                      </span>
                      <button
                        onClick={() => updateQty(item.id, item.size, item.qty + 1)}
                    ...
                    <div className="text-right">
                      ...
                      {item.qty > 1 && (
                        <p className="text-xs text-white/40 font-sans">Rs. {item.price.toLocaleString()} each</p>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="pt-4 flex justify-end">
            <button
              onClick={clearCart}
              className="text-xs text-white/40 hover:text-red-400 font-sans tracking-wide transition-colors"
            >
              Clear bag
            </button>
          </div>
        ...
        <div className="lg:sticky lg:top-24 h-fit">
          <div className="bg-black/50 border border-charcoal/10 p-6 md:p-8">
            ...
            <div className="space-y-3 text-sm font-sans border-t border-charcoal/10 pt-5">
              ...
              <p className="text-xs text-white/40">Free delivery inside Kathmandu Valley</p>
            </div>
  ...
  Checkout <ArrowRight size={14} />
            ...
            <Link
              to="/collection"
              className="block text-center mt-4 text-xs text-white/50 hover:text-white font-sans tracking-wide transition-colors"
            >
              Continue Shopping
            </Link>
          </div>
